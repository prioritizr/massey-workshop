...
\url{https://github.com/prioritizr/massey-workshop/raw/master/data.zip}.
...
data describing the spatial distributions of 32 vegetation classes
...
\NormalTok{pu_data <-}\StringTok{ }\KeywordTok{suppressWarnings}\NormalTok{(}\KeywordTok{as}\NormalTok{(}\KeywordTok{read_sf}\NormalTok{(}\StringTok{"data/pu.shp"}\NormalTok{), }\StringTok{"Spatial"}\NormalTok{))}
...
\NormalTok{veg_data <-}\StringTok{ }\KeywordTok{suppressWarnings}\NormalTok{(}\KeywordTok{stack}\NormalTok{(}\StringTok{"data/vegetation.tif"}\NormalTok{))}

\CommentTok{# verify that data are in same coordinate reference system}

\CommentTok{# manually set the coordinate reference system for veg_data to be equal to}
\CommentTok{# the coordinate reference system for pu_data,}
\NormalTok{## note that we do this because we know that the data have the same}
\NormalTok{## projection system, and normally you use would sp::spTransform or}
\NormalTok{## raster::projectRaster}
\end{Highlighting}
\end{Shaded}
